...
def _run_command(command, is_print_c=True):
    if is_print_c:
        print("CMD: {}".format(command))
    result = subprocess.run(command, shell=True, stderr=sys.stderr, stdout=sys.stdout)
...
def subprocess_popen(args, stdin=None, stdout=PIPE, stderr=None, bufsize=8388608):
    return Popen(args, stdin=stdin, stdout=stdout, stderr=stderr, bufsize=bufsize, universal_newlines=True)
